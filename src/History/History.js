...
export const useHistoryStore = defineStore('history', {
  ...
  actions: {
     ...
     */
    undo() {
      if (!this.canUndo) return console.log("无可用撤销操作")

      this.isOperating = true
      const action = this.undoStack.pop()
      console.log(`执行撤销：${action.type}`)
      try {
        action.undo() // 执行 canvasStore 提供的撤销逻辑
        this.redoStack.push(action)
        // 清空选中状态（如果需要，可通过回调让 canvasStore 处理）
        this.notifyCanvas('undo')
        const canvasStore = useCanvasStore()
        if (canvasStore && Array.isArray(canvasStore.objects)) {
          canvasStore.objects = canvasStore.objects.filter(obj => 
            obj !== null && obj !== undefined && obj.x !== undefined && obj.y !== undefined
          )
        }
      } catch (err) {
    ...
    redo() {
      ...
      try {
        action.redo() // 执行 canvasStore 提供的重做逻辑
        this.undoStack.push(action)
